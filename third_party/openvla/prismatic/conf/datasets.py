...
# [GuideVLA] Define Project Root and Default Dataset Directory
PROJECT_ROOT = Path(__file__).resolve().parent.parent.parent.parent
DEFAULT_DATASET_ROOT = Path(os.getenv("PRISMATIC_DATASET_ROOT", PROJECT_ROOT / "datasets" / "prismatic-vlms"))
...
class DatasetConfig(ChoiceRegistry):
    ...
    dataset_root_dir: Path = DEFAULT_DATASET_ROOT   # Path to dataset root directory; others paths are relative to root
    # fmt: on
...
class LLaVa_V15_Config(DatasetConfig):
    ...
    dataset_root_dir: Path = DEFAULT_DATASET_ROOT
