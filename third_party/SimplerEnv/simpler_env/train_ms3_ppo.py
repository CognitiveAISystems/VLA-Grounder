...
class Args:
    ...
    specific_objects: list[str] = None
    """List of specific objects to use in the environment (if supported)."""
    ...
    policy_type: str = "openvla" # openvla, pi0
...
class Runner:
    def __init__(self, all_args: Args):
        ...
        # policy
        if self.args.policy_type == "pi0":
            from simpler_env.policies.pi0.pi0_model import PI0Policy
            device_id = 0
            self.device = torch.device("cuda:" + str(device_id))
            self.policy = PI0Policy(all_args, device_id)
            self.alg = None 
        else:
    ...
    def _get_action(self, obs, deterministic=False):
        ...
        values, actions, logprobs = self.policy.get_action(obs, deterministic)
    ...
    def eval(self, obj_set: str) -> dict:
        ...
        obs_img, instruction, info = self.env.reset(obj_set=obj_set, specific_objects=self.args.specific_objects)
        if self.args.policy_type == "pi0":
            self.policy.wrapper.reset()
            for _ in range(self.args.episode_len / self.max_chunk):
                obs = dict(image=obs_img, task_description=instruction)
                obs["pi_0"] = info["pi_0"]

                if self.current_chunk == 0:
                    value, actions, logprob = self._get_action(obs, deterministic=True)
                action = self.actions[:, self.current_chunk, :]
                self.current_chunk = (self.current_chunk + 1) % self.max_chunk
                
                obs_img, reward, done, env_info = self.env.step(action)
                # info
                print({k: round(v.to(torch.float32).mean().tolist(), 4) for k, v in env_info.items() if k != "episode" and k != "pi_0"})
                if "episode" in env_info.keys():
                    for k, v in env_info["episode"].items():
                        env_infos[f"{k}"] += v
                
                # Update info for next step (proprio)
                info = env_info
        else:
    ...
    def render(self, epoch: int, obj_set: str) -> dict:
        ...
        obs_img, instruction, info = self.env.reset(obj_set, specific_objects=self.args.specific_objects)
        ...
        if self.args.policy_type == "pi0":
            ...
            for _ in range(self.args.episode_len):
                ...
                action = actions[:, self.current_chunk, :]
                ...
                for i in range(self.args.num_envs):
                    post_action = action #self.env._process_action(action)
                    log_image = obs_img[i].cpu().numpy()
                    log_action = post_action[i].cpu().numpy().tolist()
                    log_info = {k: v[i].tolist() for k, v in env_info.items() if k != "episode" and k != "pi_0"}
    ...
    def run(self):
        ...
        for episode in range(max_episodes):
            ...
            obs_img, instruction, info = self.env.reset(obj_set="train", same_init=self.args.use_same_init, specific_objects=self.args.specific_objects)
            ...
            for _ in tqdm(range(self.args.episode_len), desc="rollout"):
                ...
                # Update info for next step (proprio) - though collect doesn't use it yet
                info = env_info
