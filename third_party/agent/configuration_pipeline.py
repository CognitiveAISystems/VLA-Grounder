...
class EvalConfig:
    ...
    action_step: int = 4  # Number of predicted actions executed per policy call.
    ...
    def __post_init__(self):
        ...
        if self.eval_cfg is not None:
            ...
            if self.eval_cfg.env_adapter is not None:
                self.eval_cfg.env_adapter_path = f"experiments.env_adapters.{simulator_name}.{self.eval_cfg.env_adapter}"
            else:
                self.eval_cfg.env_adapter_path = f"experiments.env_adapters.{simulator_name}.BridgeSimplerAdapter"

            if simulator_name is not None:
                self.eval_cfg.simulator_path = f"experiments.envs.{simulator_name}.{simulator_name}_evaluator.{simulator_name[:1].upper() + simulator_name[1:]}Evaluator"
            else:
