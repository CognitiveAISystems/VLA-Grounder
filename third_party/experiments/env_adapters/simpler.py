...
from experiments.env_adapters.base import BaseEnvAdapter
from utils.geometry import euler2axangle, mat2euler, quat2euler, quat2mat
from utils.pipeline import process_images
...
class SimplerAdapter(BaseEnvAdapter):
    def __init__(
        ...
        # for normalization
        stats_path = Path(env_config.dataset_statistics_path)
        if not stats_path.is_absolute():
            # VLA Grounder resolves statistics relative to the vendored pipeline root.
            PROJECT_ROOT = Path(__file__).resolve().parent.parent.parent
            resolved_path = PROJECT_ROOT / stats_path
            if not resolved_path.exists():
                if (PROJECT_ROOT / "SimplerEnv" / stats_path).exists():
                    resolved_path = PROJECT_ROOT / "SimplerEnv" / stats_path
            stats_path = resolved_path
